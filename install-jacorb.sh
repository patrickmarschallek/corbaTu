...
export PATH=/opt/local/bin:/opt/local/sbin:$JACORB_HOME:$PATH

if grep "JACORB_HOME" /etc/environment -q;
then
	OriginEnv=cat /etc/environment
	Environment=$( cat <<END \
	$OriginEnv
	...
	sudo echo "$Environment" >> /etc/environment
fi
#download dependencies and installing
wget -v -O jacorb.zip http://www.jacorb.org/releases/3.2/jacorb-3.2-binary.zip
sudo unzip jacorb.zip -d $installPath
rm jacorb.zip

#build jacorb for src files
#cd $installPath/jacorb-3.2/
#ant -v
#cd ~

#write property file
touch $installPath/jacorb-3.2/etc/jacorb.properties
cat > $installPath/jacorb-3.2/etc/jacorb.properties <<END \
##
...
ORBInitRef.NameService=file:$HOME/NS_REF
...
jacorb.naming.ior_filename=$HOME/NS_Ref
